...
def plot_mult_refl(M:list[Medium], n=1000, d=1e-7):
    ks = np.linspace(1 / (d * np.e), np.e / d, n)
...
def plot_fields(w:Wave, M:list[Medium], n=100):
    ...
    ns = [cmath.sqrt(m.m * m.e) for m in M]
    ds = [m.d for m in M]
    ds[0] = np.inf
    ds[-1] = np.inf
    sol = tm.coh_tmm('s', ns, ds, abs(w.k[1] / cmath.sqrt(w.k@w.k)),cmath.sqrt(w.k@w.k))
    ks = sol['kz_list']
    A = sol['vw_list'][0] + sol['vw_list'][1]
    rs = sol['r']
    ts = sol['t']

    ws = [Wave(w.a*rs, w.k, w.p)] + [Wave(A[idx], np.array([w.k[0], ks[idx+1]]), w.p) for idx in range(len(A))] + [Wave(w.a*ts, np.array([w.k[0], ks[-1]]), w.p)]
