...
%
  ...
  s = 1;    % stepsize for neighbour search
  ca = 1:K-s; % candidates that may be unified with their s-th neighbour
  while 1
    
    % Since intervals are sorted according to the infimum of their first
    ...
    if ~isempty(ui)
      ...
      uis = ui+s;  % for efficiency we keep the indices of the neighbour
      for i = 1:N
    ...
    if ~isempty(ui)
      ...
      % then we mark the neighbours ui+s as redundant
      rem(ui+s) = true;
    ...
    % remove last candidate if possible neighbour lies out of range
    if ca(end)+s>K
