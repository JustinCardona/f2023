...
% modified 07/16/18     S.M. Rump  (-inf)-(-inf) for error (thanks to Florian Buenger)
%
...
%                  length(err.mant) , length(err.exp) )
    ...
    if any(index1)
      dexp = err.exp(index1)-Err.exp(index1);
      dexp(isnan(dexp)) = 0;
      Err.mant(index1) = Err.mant(index1) + ...
        ( realmin + ...
          err.mant(index1) .* INTLAB_LONG_BETA.^dexp );
    end
