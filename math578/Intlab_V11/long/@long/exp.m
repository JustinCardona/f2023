...
% modified 05/30/18     S.M. Rump  vector input (thanks to Florian Bünger)
%
  ...
  if any( index )
    ...
    while any(index)
      E(index) = E(index) + 1;      
%VVVV X(index) = X(index)/2;
      s.type = '()'; s.subs = {index}; X = subsasgn(X,s,subsref(X,s)/2);
%AAAA Matlab bug fix    
      index = ( X.exponent>0 );
    end
  else
  ...
  if large
    index = ( E~=0 );
    while any(index)
%VVVV C(index) = C(index)*C(index);
      s.type = '()'; s.subs = {index};
      Cindex = subsref(C,s);
      C = subsasgn(C,s,Cindex*Cindex);
      % E(index) = E(index) - 1;
      E = subsasgn(E,s,subsref(E,s)-1);
%AAAA Matlab bug fix    
      index = ( E~=0 );
    end
  end
