...
% modified 12/30/17  S.M. Rump  kappa outside X0
% modified 06/25/18  S.M. Rump  abs of Hessian
% modified 07/05/18  S.M. Rump  intval function evaluation
% modified 07/12/18  S.M. Rump  check for kappa
% modified 07/15/18  S.M. Rump  progress
% modified 07/23/18  S.M. Rump  loop improvement
% modified 07/24/18  S.M. Rump  new and fast removeDoubleEntries due to
%                                  Marko Lange, thanks a lot!
% modified 07/25/18  S.M. Rump  try for closing window
% modified 07/28/18  S.M. Rump  sparse gradient
...
%GAMMAin  expandiertes kappa in X0
...
%ListS    noch nicht entschiedene Boxen
  ...
  INTLAB_CONST.TIMER = clock;
  if ischar(INTLAB_NLSS.SEE)
    INTLAB_NLSS.OPTIMWINDOW = figure;
    try
  ...
  for iter = 1:INTLAB_NLSS.NIT    % main loop
    ...
    INTLAB_NLSS.NIT_CURRENT = iter;
    popup = ( etime(clock,INTLAB_CONST.TIMER)>INTLAB_CONST.MINTIME );
    if ( INTLAB_NLSS.SEE==1 ) | popup
      str = sprintf('Pass %d of %d, treating next %d sublists\n', ...
  ...
  if ~isempty(INTLAB_NLSS.kappa)
    % kappa completely outside X0: there is root, but outside
    outX0 = any( bsxfun(@gt,INTLAB_NLSS.kappa.sup(1:N,:),INTLAB_NLSS.X0.sup(1:N)) | ...
                 bsxfun(@lt,INTLAB_NLSS.kappa.inf(1:N,:),INTLAB_NLSS.X0.inf(1:N)) ,1);
    if any(outX0)
      INTLAB_NLSS.GAMMA(:,outX0) = [];
      INTLAB_NLSS.GAMMAin(:,outX0) = [];
      INTLAB_NLSS.kappa(:,outX0) = [];
    end
    if INTLAB_NLSS.GLOBOPT || INTLAB_NLSS.CONSTRAINT
      ...
      if any(inX0) & ( ~isempty(ListS) )
          index = inX0 & INTLAB_NLSS.ismin;
          % extract elements of ListS corresponding to kappa
          GammaInf = INTLAB_NLSS.GAMMA.inf(1:N,INTLAB_NLSS.GAMMAin);
          GammaSup = INTLAB_NLSS.GAMMA.sup(1:N,INTLAB_NLSS.GAMMAin);
          ListSinf = inf(ListS(1:N,:));
          ListSsup = sup(ListS(1:N,:));
          for j=find(index)
            % all(in( ListS , INTLAB_NLSS.GAMMA(:,j) ))
            dame = ( all(bsxfun(@ge,ListSinf,GammaInf(:,j)),1) & ...
                     all(bsxfun(@le,ListSsup,GammaSup(:,j)),1) );
            if any(dame(:))
              ListS(:,dame) = [];
              if isempty(ListS)
                break
      ...
      if any(inX0) & ( ~isempty(ListS) )
        % extract elements of ListS corresponding to kappa
        GammaInf = INTLAB_NLSS.GAMMA.inf(:,INTLAB_NLSS.GAMMAin);
        GammaSup = INTLAB_NLSS.GAMMA.sup(:,INTLAB_NLSS.GAMMAin);
        ListSinf = inf_(ListS);
        ListSsup = sup(ListS);
        for j=find(inX0)
          % all(in( ListS , INTLAB_NLSS.GAMMA(:,j) ))
          dame = any( all(bsxfun(@ge,ListSinf,GammaInf(:,j)),1) & ...
                      all(bsxfun(@le,ListSsup,GammaSup(:,j)),1) );
         if any(dame(:))
            ListS(:,dame) = [];     
            if isempty(ListS)
...
%   ListS = collectList(ListS);  % mu better before union
  ...
  if  INTLAB_NLSS.NLSSALL
    ...
    ListData.GAMMAin = INTLAB_NLSS.GAMMAin;
...
function [listL,listB] = bisection(v,level,listL,listB,dummy)
  ...
  if INTLAB_NLSS.GLOBOPT
    ...
    if ( INTLAB_NLSS.SEE==1 ) | popup
    str = [ ...
      'Iteration ' int2str(INTLAB_NLSS.NIT_CURRENT) ...
      ' out of ' int2str(INTLAB_NLSS.NIT) ', ' ...
      'Level ',int2str(level) ', Length(L) ' sprintf('%d',size(v,2)) ...
...
%       ydx = squeeze(y.dx)';     % problems in older Matlab versions
      if size(v,2)==1
        ...
        if issparse(y)      % y.dx for full gradient
          yfull = full(y);
          ydx = permute(yfull.dx,[2 3 1])'; 
        else
...
%     [dummy,index] = sort(yff.inf - sum(mag(ydx).*rad(v),1));
  ...
  if ( ~INTLAB_NLSS.NLSSALL ) && ismember(level/N,1:2:INTLAB_NLSS.ND-1)
    if ~isempty(INTLAB_NLSS.kappa)
      index = discard(v,INTLAB_NLSS.GAMMA(:,INTLAB_NLSS.GAMMAin),1);
      if any(index)
        if ischar(INTLAB_NLSS.SEE)
...
function localstrategy(v)
  ...
  if ~isempty(xs)
    ...
    if ~isempty(xs)
      ...
      if any(kappa)
        indexin = all(bsxfun(@ge,kappa.inf(1:N,:),INTLAB_NLSS.X0(1:N,:).inf),1) & ...
          all(bsxfun(@le,kappa.sup(1:N,:),INTLAB_NLSS.X0(1:N,:).sup),1);
        if any(indexin) & INTLAB_NLSS.CONSTRAINT
          INTLAB_NLSS.feasible = 1;
        end
        ...
        INTLAB_NLSS.GAMMAin = logical([ INTLAB_NLSS.GAMMAin indexin ]);
    ...
    while any(cont) && ( j<jmax )
      ...
      if j>1
        ...
        if any(ok)
          ...
          if isempty(INTLAB_NLSS.param)
            supyxok = sup(feval(INTLAB_NLSS.F,intval(xsok)));
          else
            supyxok = sup(feval(INTLAB_NLSS.F,intval(xsok),INTLAB_NLSS.param{:}));
          end
    ...
    while any(cont) && ( j<jmax )
      ...
      if ( INTLAB_NLSS.SEE==1 ) | popup
        str = [ ...
                ...
                ' out of ' int2str(INTLAB_NLSS.NIT) ', ' ...
                ' bisection depth ' sprintf('%d',INTLAB_NLSS.BISECT_DEPTH) ', ' ...
                ' local strategy, ' int2str(j) '-' ending(j) ' iteration, ' ...
                int2str(size(xs,2)) ' candidates'];
        if INTLAB_NLSS.SEE==1
          disp(str)
        end
        if popup
      ...
      if INTLAB_NLSS.DERIV
        ...
        if N==1
          ...
          if issparse(y)      % y.dx for full gradient
            yfull = full(y);
            ydx = reshape(permute(yfull.dx,[1 3 2]),N2,K);
          else
...
function res = ending(k)
% choose correct ending
  switch k
    case 1, res = 'st';
    case 2, res = 'nd';
    case 3, res = 'rd';
...
function plotres(x,color)
  global INTLAB_NLSS
  try       % careful if windows is closed
    figure(INTLAB_NLSS.OPTIMWINDOW)
  catch
    return
  end
  N = INTLAB_NLSS.N;        % number of unknowns
...
function [B, Isel] = removeDoubleEntries(A)
  % A given, eliminate common points
  % Isel: remaining indices
  % A(:,Isel) = B
  % Fast version due to Marko Lange, thanks a lot!
  tol = 1e-6;
  if isempty(A)
    B = A;
    Isel = [];
  ...
  % dimension: k elements with n coordinates
  [n, k] = size(A);
  
  %% fast approach to find bounds for possible clustering
  
  % sorting each coordinate individually
  [At, I] = sort(A', 1);
  I = bsxfun(@plus, I, 0:k:n*k-k);
  % cluster indexing for original data
  Ct = zeros(k, n);
  Ct(I) = cumsum([ones(1, n); abs(diff(At, 1, 1)) > tol], 1);
  % stable ordering from back to front
  [Ct, isort] = sortrows(Ct);
  
  % starting indices for possible clusters
  %   ordered according to isort
  %   computed by unifying the cluster bounds for all coordinates
  idx = [1; find(sum(abs(diff(Ct, 1, 1)),2)) + 1];
  
  % input data ordered according to initial clustering
  At = A(:,isort);
  
  % output data
  %   initialized as cell for faster processing: concatenation at the end
  B = cell(0);
  
  % index vector for cluster assignment
  I = zeros(k, 1);
  % offset for current cluster positions
  offs = 0;
  
  % indices of selected cluster centers
  %   initialized as cell for faster processing: concatenation at the end
  Isel = cell(0);
  
  
  %% actual clustering exploiting initial cluster bounds
  while ~isempty(At)
      
      % index vector identifying possible cluster assignment
      ind = zeros(size(At, 2), 1);
      ind(idx) = 1;
      ind = cumsum(ind);
      
      % extract centers  (= starting elements of each possible cluster)
      B{end+1} = At(:,idx);
      Isel{end+1} = isort(idx);
      
      % find all elements actually clustered around centers in B{end}
      irem = find(all(abs(B{end}(:,ind) - At) <= tol, 1));
      
      % update I with newly found cluster assignments
      I(isort(irem)) = offs + ind(irem);
      % update offset for next set of cluster centers
      offs = offs + size(B{end}, 2);
      
      % remove all elements actually assigned to some center in B{end}
      At(:,irem) = [];
      isort(irem) = [];
      ind(irem) = [];
      
      % find new starting indices for possible clustering
      idx = [1; find(diff(ind)) + 1];
      
  end
  
  % unify all cluster centers and their indices
  B = horzcat(B{:});
  Isel = vertcat(Isel{:});
end  % function removeDoubleEntries


function [x,Index] = removeDoubleEntries_old(x)
  ...
  if K>0
    ...
    Index = false(1,K);
    if K>Kmax           % careful with memory
      for KK=1:Kmax:K
        v = KK:min(K,(KK+Kmax-1));
        k = length(v);
        Q = true;
        for i=1:n
          M = repmat(x(i,v),k,1);
...
end  % function removeDoubleEntries_old
...
function L = NewtonTest(L,dummy)
  ...
  if nargin==1
    ...
    try         % take care of Hessian for abs
