...
% modified 03/18/18  S.M. Rump  comment, fminsearch options
% modified 04/17/18  S.M. Rump  spell check
% modified 06/27/18  S.M. Rump  abs function
%
  ...
  if refine                             % refinement call
    ...
    if nargin<4
      ...
      eval([str ');'])
      x = fminsearch(fparam,x0.mid,optimset('Display','off'));
    end
    if all(in(x,x0))
      if nargin<4
    ...
    INTLAB_NLSS.GAMMA = intval([]);  % expanded kappa
    INTLAB_NLSS.GAMMAin = [];        % expanded kappa in CurrentX0
