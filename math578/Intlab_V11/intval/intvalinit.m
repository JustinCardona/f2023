...
% modified 08/24/12     S.M. Rump  SECOND_STAGE superfluous due to redesign of verifylss
...
% modified 12/26/17     S.M. Rump  sparse bug
% modified 04/17/18     S.M. Rump  spell check
% modified 04/17/18     S.M. Rump  INTLAB_SPARSE_BUG
% modified 06/18/18     S.M. Rump  suppress output
% modified 07/15/18     S.M. Rump  mintimer for progress
%
  ...
  case 'veryfirstinit'
    ...
    % check for Matlab sparse bug: sparse(0)*NaN=0
    INTLAB_SPARSE_BUG = ~any(any(isnan(speye(2)*sparse([NaN;0]))));
    ...
    % Minimum time before popup
    INTLAB_CONST.MINTIME = 5;
