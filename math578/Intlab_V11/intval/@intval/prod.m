...
% modified 05/16/18     S.M. Rump  point vector and Buenger's correction
% modified 05/20/18     S.M. Rump  sign for N-dim arrays (thanks to Florian Buenger)
%
  ...
  if isreal(a)                      % real input
    if all(all(a.inf==a.sup))       % point interval input
      ...
      setround(1)
      csup = prod(aa,dim);
      c = prod(sign(a.inf),dim) .* intval(cinf,csup,'infsup');
    ...
    %VVVV c = a(1,:);
    s.subs{dim} = 1; c = subsref(a,s);
    %AAAA Matlab V5.2 bug fix
    for i=2:m
      %VVVV c = c .* a(i,:);
      s.subs{dim} = i; c = c .* subsref(a,s);
      %AAAA Matlab V5.2 bug fix
    end
  end
