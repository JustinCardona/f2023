...
%
%  plotintval(X)  or  plotintval(X,color)  or  plotintval(X,color,kmax)
%
...
%  factor = 0.2; A = factor*[1 -2;3 -4], z = [1;1], close
...
% modified 07/26/18     S.M. Rump  phase plot
%
  ...
  if a.complex          % complex input
    ...
    if dim<3                            % 1- or 2-dimensional
      if isequal(fillcolor,'o')
        ...
        if dim==2           % for phase plot
          amid = 0.5*(ainf+asup);
          try
            plot(amid(1,:),amid(2,:),['.' color])
          catch             % random color
            plot(amid(1,:),amid(2,:),['.' rand(1,3)])
          end
        end
        if isequal(color,'n')
          p = plot(args{:});
