...
% modified 12/25/17     S.M. Rump  sparse nan
% modified 04/17/18     S.M. Rump  spell check
% modified 04/17/22     S.M. Rump  0*inf
%
  ...
  persistent first                  % initialized empty
  if isempty(first)                 % check for NaNs
    aisnan = any(isnan(a),2);
    bisnan = any(isnan(b),1);
    if any(aisnan) | any(bisnan) | ...   % treat NaNs separately
       any(any(isinf(a))) | any(any(isinf(b))) % take care of 0*inf
      as = a;
      %VVVV as(aisnan,:) = 0;
      s.type = '()'; s.subs = {aisnan,':'}; as = subsasgn(as,s,0);
      %AAAA Matlab V5.2 bug fix
      bs = b;
      %VVVV bs(:,bisnan) = 0;
      s.type = '()'; s.subs = {':',bisnan}; bs = subsasgn(bs,s,0);
      %AAAA Matlab V5.2 bug fix
      first = 0;
      % The following takes care of 0*inf=0. It may slow down the computation, 
      % however that is only for NaN's occurring.
      if nargin==2
%         r = mtimes(as,bs);
        r = mtimes(sparse(as),sparse(bs));
      else
%         r = mtimes(as,bs,0);
        r = mtimes(sparse(as),sparse(bs),0);
      end
      if ( ~issparse(as) ) | ( ~issparse(bs) )
        r = full(r);
      end
      first = [];
      %VVVV r(aisnan,:) = NaN;
  ...
  elseif ~isa(b,'intval')                 % b is double
    ...
    if a.complex || b.complex             % complex case
      ...
      elseif bthin
        ...
        c.inf = -(crad + (-amid) * b.inf);% equivalent to  setround(-1); c.inf = amid * b.inf - crad; 
      else                                  % IR * IR
