...
% modified 07/17/18     S.M. Rump  intval type cast (thanks to 
%                                    Florian Buenger)
% modified 02/07/19     S.M. Rump  complex integer powers (thanks to JB, 
...
%
  ...
  if isreal(b)
    ...
    if isa(b,'double') && numel(b)==1  % real integer power
      if b==round(b)
        if b==0
          r = typeadj( ones(size(a)) , typeof(a) );
        else                        % b is integer
          if ~isreal(a)
            r = intpowerc(a,abs(b)-1);
            if b<0
              r = 1./r;
            end
            return
...
function r = intpowerc(a,b)
% integer power a^(b+1) by binary multiplication, b>0; a supposed to be interval
  r = a;
  while b>0
    if mod(b,2)==1
      r = r.*a;
...
% Rounding is already switched to upwards by calling function 'power'. 
  rinf = ones(size(ainf));
  rsup = rinf;
  b_0 = ( b==0 );
  if any(b_0(:))
...
% Rounding is already switched to upwards by calling functions 'power1' and 'power'. 
 r1 = a1;
 r2 = a2;
 b = b(:)-1;
 while any(b>0)
