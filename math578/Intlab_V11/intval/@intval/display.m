function str = display(c,name,restricted)
...
% modified 07/26/18     S.M. Rump  output argument
%
  ...
  if nargout==0
    ...
    if isequal(INTLAB_INTVAL_DISPLAY,'DisplayInfsup')
      str = infsup(c,name,restricted);
    elseif isequal(INTLAB_INTVAL_DISPLAY,'DisplayMidrad')
      str = midrad(c,name,restricted);
    else
      str = disp_(c,name,restricted);	    % covers disp_ and disp__
    end
  end
