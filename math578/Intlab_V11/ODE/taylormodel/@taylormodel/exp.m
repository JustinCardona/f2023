function r = exp(a)
%EXP  Taylor model exponential exp(a)
%
%   r = exp(a) 
...
% modified 08/11/16     F. Buenger  "intval"-components --> intval-like structures 
    ...
    for i = 1:S_a(1)
        for j = 1:S_a(2)
            ...
            if RECMODE ~= 2
                ...
                if ODEMODE == 1
                    c = intval(c,c,'infsup');
                    C = INTLAB_ODE_INVERSE_FACTORIAL.INTVAL(1:n+2);
                else
                    C = INTLAB_ODE_INVERSE_FACTORIAL.FLP(1:n+2);
                end
                
                % Taylor polynomial p(x) of degree n of the function phi(x):=exp(x) at x_0:=c
                % evaluated for x:=a. See [E], p.23 and p.29,30.
                %
                % p(x) := exp(c) * { 1 + (x-c) + (x-c)^2/2 + ... + (x-c)^n/n!}
                %
                ...
                if ODEMODE == 1 % verified mode.
                    ...
                    % See [E], p.30, error term caused by the remainder of the Taylor expansion
                    % r(x) := exp(c) (x-c)^(n+1)/(n+1)! * exp(theta*(x-c)),
                    %       = exp(c) h^(n+1)/(n+1)! * exp(theta*h) , theta in (0,1)
                    h_ = I_y-c;
                    I_ry = exp(c) * C(n+2) * h_^(n+1) * exp(theta*h_); % See [E], p.30.
                ...
                C = INTLAB_ODE_INVERSE_FACTORIAL.INTVAL(1:n+2); % From the record write call in the probably far past, still
                % length(INTLAB_ODE_INVERSE_FACTORIAL.FLP) >= (n + 2) holds true.
                h = subtract_constant_term(a_,i0); % fast computation of a_ - c
