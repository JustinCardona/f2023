    ...
    for i = 1:S_a(1)
        for j = 1:S_a(2)
            ...
            if RECMODE ~= 2
                ...
                % Taylor polynomial p(x) of degree n of the function phi(x):=sin(x) at x_0:=c
                % evaluated for x:=a_. See [E], p.23 and p.29,30,34.
                %
                % p(x) := sin(c) + cos(c)*h - 1/2!*sin(c)*h^2 - 1/3!*cos(c)*h^3 ...
                %
                ...
                if RECMODE == 1 % record write mode
                    ...
                    % Store c,i0,C,a_.image in record list.
                    push_reclist('n', n);
                    push_reclist('c', c);
                    push_reclist('i0', i0);
                    push_reclist('C', C);
                ...
                if ODEMODE == 1 % verified mode.
                    ...
                    h_ = I_y-c;
                    if even(n)
                        I_ry = F(n+2)*h_^(n+1)*cos(c+theta*h_); % See [E], p.30,34.
                    else
                        I_ry = F(n+2)*h_^(n+1)*sin(c+theta*h_); % See [E], p.30,34.
                    end
                    x = mod(n,4);
                    if x == 1 || x == 2
                        I_ry = -I_ry;
                ...
                if even(n)
                    I_ry = F * h_^(n+1)*cos(c+theta*h_); % See [E], p.30,34.
                else
                    I_ry = F * h_^(n+1)*sin(c+theta*h_); % See [E], p.30,34.
                end
                x = mod(n,4);
                if x == 1 || x == 2
                    I_ry = -I_ry;
