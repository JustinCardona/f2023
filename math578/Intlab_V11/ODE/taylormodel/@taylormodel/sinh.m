function r = sinh(a)
%SINH  Taylor model hyperbolic sine sinh(a)
%
%   r = sinh(a)
    ...
    for i = 1:S_a(1)
        for j = 1:S_a(2)
            ...
            if RECMODE ~= 2
                ...
                % Taylor polynomial p(x) of degree n of the function phi(x):=sinh(x) at x_0:=c
                % evaluated for x:=a_. See [E], p.23 and p.29,30,35.
                %
                % p(x) := sinh(c) + cosh(c)*h + 1/2!*sinh(c)*h^2 + 1/3!*cosh(c)*h^3 ...
                %
